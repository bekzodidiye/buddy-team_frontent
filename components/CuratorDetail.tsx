  ...
  return (
    <div className="fixed inset-0 z-[110] bg-[#0a0a0c] overflow-y-auto animate-in fade-in duration-300">
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-12 py-8 md:py-20 min-h-screen">
        ...
        {activeTab === 'profile' ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12 animate-in fade-in duration-500 max-w-7xl mx-auto"><div className="lg:col-span-1"><div className="glass border border-white/5 rounded-3xl p-6 md:p-8 shadow-xl"><div className="relative aspect-square rounded-3xl overflow-hidden mb-6 md:mb-8 border-4 border-white/5 shadow-2xl group mx-auto max-w-[300px]"><img src={curator.avatar} alt={curator.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" /><div className="absolute inset-0 bg-gradient-to-t from-[#0a0a0c] via-transparent to-transparent opacity-60"></div></div><div className="text-center mb-8 md:mb-10"><h2 className="text-3xl md:text-4xl font-black text-white mb-2 tracking-tighter">{curator.name}</h2><p className="text-indigo-400 font-black uppercase tracking-[0.2em] text-[10px] md:text-xs mb-6">{curator.role}</p><div className="flex flex-wrap justify-center gap-1.5 md:gap-2">{curator.skills.map((skill, index) => (<span key={`${skill}-${index}`} className="px-3 py-1.5 bg-white/5 border border-white/5 rounded-xl text-[8px] md:text-[9px] font-black text-slate-500 uppercase tracking-tight">{skill}</span>))}</div></div><div className="grid grid-cols-2 gap-3 md:gap-4"><div className="p-4 md:p-6 bg-white/5 rounded-2xl border border-white/5 text-center"><Users className="w-4 h-4 md:w-5 md:h-5 text-indigo-400 mx-auto mb-2" /><p className="text-[8px] md:text-[9px] font-black uppercase text-slate-500 tracking-widest mb-1">O'quvchilar</p><p className="text-xl md:text-2xl font-black text-white">{curatorStudents.length}</p></div><div className="p-4 md:p-6 bg-white/5 rounded-2xl border border-white/5 text-center"><Star className="w-4 h-4 md:w-5 md:h-5 text-yellow-500 fill-current mx-auto mb-2" /><p className="text-[8px] md:text-[9px] font-black uppercase text-slate-500 tracking-widest mb-1">Reyting</p><p className="text-xl md:text-2xl font-black text-white">5.0</p></div></div></div></div><div className="lg:col-span-2 space-y-8 md:space-y-12"><div className="glass border border-white/5 rounded-3xl p-6 md:p-14 relative overflow-hidden shadow-xl"><div className="absolute top-0 right-0 p-12 opacity-[0.03] text-indigo-500 hidden md:block"><Briefcase className="w-64 h-64" /></div><h3 className="text-xl md:text-2xl font-black text-white mb-8 md:mb-10 flex items-center space-x-3 md:space-x-4"><div className="w-10 h-10 md:w-12 md:h-12 bg-indigo-600/20 rounded-xl flex items-center justify-center text-indigo-500 border border-indigo-500/20"><Zap className="w-5 h-5 md:w-6 md:h-6" /></div><span>Soha va Tajriba</span></h3><div className="space-y-8 md:space-y-10 relative z-10"><div><h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-4 ml-1">Mutaxassis haqida</h4><p className="text-lg md:text-2xl text-slate-300 leading-relaxed font-medium tracking-tight">{curator.longBio}</p></div><div className="p-6 md:p-10 bg-indigo-500/5 border border-indigo-500/10 rounded-2xl relative overflow-hidden"><div className="absolute top-0 left-0 w-1 h-full bg-indigo-600"></div><h4 className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] text-indigo-400 mb-4 flex items-center gap-2"><Info className="w-3.5 h-3.5 md:w-4 h-4" />Mutaxassislik falsafasi</h4><p className="text-slate-400 text-base md:text-lg leading-relaxed italic font-medium">"{curator.fieldDescription}"</p></div></div></div><div className="relative group"><div className="absolute -left-3 -top-3 md:-left-6 md:-top-6"><Quote className="w-16 h-16 md:w-24 md:h-24 text-indigo-600/10" /></div><div className="glass border border-white/5 rounded-3xl p-8 md:p-24 text-center bg-gradient-to-br from-indigo-600/[0.03] to-purple-600/[0.03] shadow-xl"><p className="text-xl sm:text-2xl md:text-5xl font-black text-white leading-tight italic tracking-tighter">"{curator.motivationQuote}"</p></div></div></div></div>
        ) : (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-12 md:space-y-20">
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 md:gap-8 text-center lg:text-left">
              ...
              <div className="flex flex-col items-center lg:items-end gap-3 w-full md:w-auto">
                 {/* Week Selector */}
                 <div className="w-full md:w-[280px] flex items-center justify-between p-2 glass rounded-2xl border border-white/5">
            ...
            <div className="space-y-6 md:space-y-8">
               ...
               <div className="glass rounded-3xl border border-white/5 shadow-2xl bg-[#0a0a0c]/40 overflow-hidden">
                  <div className="hidden xl:block w-full overflow-x-auto">
                    <table className="w-full text-left border-collapse table-fixed min-w-[1200px]">
                      ...
                      <tbody className="divide-y divide-white/5">{filteredPlans.length > 0 ? filteredPlans.map((item) => (<tr key={item.id} className="hover:bg-white/[0.03] transition-colors"><td className="px-10 py-9 align-middle"><div className="flex items-center space-x-4"><div className="w-10 h-10 rounded-xl bg-indigo-600/20 flex items-center justify-center font-black text-indigo-400 text-xs border border-indigo-500/20">{item.studentName[0]}</div><span className="font-black text-white text-[14px] tracking-tight">{item.studentName}</span></div></td><td className="px-4 py-9 align-middle"><span className="text-[10px] font-bold text-indigo-400 flex items-center gap-1.5 uppercase"><Calendar className="w-4 h-4" />{formatMeetingDay(item.meetingDay)}</span></td><td className="px-4 py-9 align-middle"><p className="text-[13px] text-slate-300 font-medium leading-relaxed">{item.weeklyGoal}</p></td><td className="px-4 py-9 align-middle"><p className={`text-[13px] leading-relaxed ${item.difficulty === 'Yo\'q' ? 'text-slate-600' : 'text-orange-400 font-bold'}`}>{item.difficulty}</p></td><td className="px-4 py-9 align-middle"><p className="text-[13px] text-indigo-300 font-medium italic opacity-90 leading-relaxed">{item.solution}</p></td><td className="px-4 py-9 align-middle"><span className={`inline-flex items-center px-4 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${getStatusStyle(item.status)}`}>{item.status}</span></td><td className="px-6 py-9 text-center align-middle">{item.attended ? (<UserCheck className="w-6 h-6 text-green-400 mx-auto" />) : (<UserX className="w-6 h-6 text-red-400/50 mx-auto" />)}</td></tr>)) : (<tr><td colSpan={7} className="py-24 text-center text-slate-700 font-black uppercase tracking-widest text-xs">Ushbu mavsum va hafta uchun rejalar yo'q</td></tr>)}</tbody>
                    </table>
                  </div>
                       ...
                       <div key={item.id} className="p-6 bg-white/5 border border-white/5 rounded-2xl space-y-5 shadow-xl">
                          <div className="flex items-center justify-between border-b border-white/5 pb-4">
                             <div className="flex items-center gap-3">
                                ...
                                <div>
                                   <span className="font-black text-white text-base block">{item.studentName}</span>
                                   <span className="text-[8px] text-slate-500 uppercase font-bold tracking-widest">{formatMeetingDay(item.meetingDay)}</span>
                                </div>
                             </div>
                          ...
                          <div className="space-y-4">
                             <div className="space-y-1">
                                ...
                                <p className="text-[13px] text-slate-300 leading-relaxed font-medium bg-white/5 p-3 rounded-xl">{item.weeklyGoal}</p>
                             </div>
                             ...
                             <div className="grid grid-cols-1 gap-3">
                               <div className="space-y-1">
                                  ...
                                  <div className="p-3 bg-orange-500/5 border border-orange-500/10 rounded-xl">
                                     <p className={`text-[12px] leading-relaxed font-bold ${item.difficulty === 'Yo\'q' ? 'text-slate-600 italic' : 'text-orange-300'}`}>{item.difficulty}</p>
                                  </div>
                               </div>
                               ...
                               <div className="space-y-1">
                                  ...
                                  <div className="p-3 bg-green-500/5 border border-green-500/10 rounded-xl">
                                     <p className="text-[12px] text-green-300 leading-relaxed italic font-medium">{item.solution}</p>
                                  </div>
                               </div>
                             </div>
                          </div>

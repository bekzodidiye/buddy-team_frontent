...
import { Menu, X, Users, Home, Layout, Mail, Sparkles, LogIn, LogOut, UserCircle, ShieldAlert, Bell } from 'lucide-react';
...
const Navbar: React.FC<NavbarProps> = ({ currentPage, onNavigate, onAuthNavigate, user, onLogout, isRegistrationOpen = true, unreadCount = 0 }) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-[9999] transition-all duration-500 ${
      isScrolled || isMenuOpen ? 'bg-[#0a0a0c] border-b border-white/10 py-3 shadow-[0_10px_40px_rgba(0,0,0,0.8)]' : 'bg-transparent py-4 md:py-6'
    }`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          <div onClick={() => handleLinkClick('home')} className="flex items-center space-x-2 md:space-x-3 group cursor-pointer shrink-0">
            <div className="relative w-10 h-10 md:w-12 md:h-12 flex items-center justify-center bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 rounded-xl md:rounded-2xl shadow-lg group-hover:scale-110 transition-transform duration-300 border border-white/20">
          ...
          <div className="hidden lg:flex items-center space-x-1">
            {navLinks.map((link) => (
              <button
                ...
                className={`px-4 py-2 text-sm font-semibold rounded-xl transition-all flex items-center space-x-2 relative ${
                  (link.id === 'dashboard' || link.id === 'admin') 
            ...
            {navLinks.map((link) => (
              <button
                ...
                className={`flex items-center space-x-4 p-5 rounded-2xl text-lg font-black tracking-tight transition-all relative ${
                  currentPage === link.id ? 'text-indigo-400 bg-white/10 shadow-inner' : 'text-slate-300 hover:bg-white/5'
                }`}
              >

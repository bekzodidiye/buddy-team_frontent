...
import { Github, Twitter, Sparkle, UserCheck, Search, Users, UserX, AlertCircle, Link2 } from 'lucide-react';
...
const Team: React.FC<TeamProps> = ({ user, onAssignCurator, customMembers, studentsData = [], highlights = [], seasons = [], activeSeasonId = '1' }) => {
  ...
  return (
    <section id="team" className="py-32 relative min-h-[60vh]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {isStudentChoosing && displayedMembers.length > 0 && (
          <div className="mb-12 p-6 md:p-10 glass border-dashed border border-indigo-500/20 shadow-xl rounded-3xl text-center animate-in fade-in zoom-in duration-500">
        ...
        {displayedMembers.length > 0 ? (
          <div className={`grid grid-cols-1 ${displayedMembers.length === 1 ? 'max-w-md mx-auto' : 'sm:grid-cols-2 lg:grid-cols-3'} gap-8 md:gap-12`}>
            {displayedMembers.map((member) => (
              ...
              >
                ...
                <div className="relative bg-[#121214] border border-white/5 shadow-2xl rounded-3xl p-6 md:p-8 transition-all duration-500 group-hover:border-indigo-500/50 group-hover:-translate-y-4 overflow-hidden">
                      ...
                      <div className="flex space-x-3">
                        {member.socialLinks && member.socialLinks.length > 0 ? (
                          member.socialLinks.map((link, index) => (
                            ...
                            >
                              {link.iconUrl ? (
                                <img src={link.iconUrl} alt="social" className="w-full h-full object-cover" />
                              ) : (
